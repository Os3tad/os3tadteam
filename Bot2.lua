...
function sleep(s)
  local delay = redis:get("bot1delay") or 5
...
function get_bot (i, adigram)
  function bot_info (i, adigram)
    redis:set("bot1id",adigram.id_)
    if adigram.first_name_ then
      redis:set("bot1fname",adigram.first_name_)
    end
    if adigram.last_name_ then
      redis:set("bot1lanme",adigram.last_name_)
    end
    redis:set("bot1num",adigram.phone_number_)
    return adigram.id_
  end
          ...
          if not redis:get("bot1maxjoin") then
            if redis:scard("bot1goodlinks") ~= 0 then 
              ...
              for x,y in pairs(links) do
                ...
                local maxsg = redis:get("bot1maxsg") or 200
                if tonumber(sgps) < tonumber(maxsg) then
                  tdcli_function({ID = "ImportChatInviteLink",invite_link_ = y},process_join, {link=y})
                    if x == 4 then redis:setex("bot1maxjoin", 60, true) return end
                  end
                end
              end
            end
                ...
                if is_adigram(msg) then 
                  ...
                  elseif text:match("[Rr]efresh") or text:match("بازرسی") then
                    ...
                    tdcli_function({
                          ...
                          redis:set("bot1contacts", adigram.total_count_)
                        end, nil)
                              ...
                              elseif text:match("(setusername) (.*)") or text:match("(تنظیم یوزرنیم) (.*)") then
                                ...
                                elseif text:match("(delusername)") or text:match("(حذف یوزرنیم)") then
                                  ...
                                  elseif msg.content_.ID == "MessageContact" then
                                    if redis:sismember("bot1admin",msg.sender_user_id_) then
